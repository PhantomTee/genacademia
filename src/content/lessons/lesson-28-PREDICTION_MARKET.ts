...
Smart contracts are hard to debug — there's no \`print()\` that shows in a terminal. GenLayer provides \`gl.emit_debug(message)\` which writes to the **Studio debug console** during development and testing, and is stripped silently in production deployments.
...
def place_bet(self, outcome: str) -> None:
    ...
    gl.emit_debug(
        f"place_bet: {gl.message.sender_address} bet {outcome} "
        f"with {gl.message.value} wei"
    )
    ...

@gl.public.write
def resolve_and_distribute(self) -> None:
    gl.emit_debug(f"resolve_and_distribute: starting, status={self.status}")
    ...
...
def claim_winnings(self) -> None:
    ...
    gl.emit_debug(
        f"claim_winnings: sending {share} wei to {caller}"
    )
    gl.send(caller, share)
\`\`\`

Good debug messages include:
- **Who** is calling (sender address)
- **What** values are involved (amounts, outcomes)
- **Where** in the control flow (function name, key conditions)

Because \`emit_debug\` has zero production cost, it's safe to leave descriptive messages throughout the contract — they only appear in Studio's log panel and are never stored on-chain.`,
...
class PredictionMarket(gl.Contract):
    ...
    def place_bet(self, outcome: str) -> None:
        ...
        # TODO: add gl.emit_debug logging here
    ...
    def claim_winnings(self) -> None:
        ...
        # TODO: add gl.emit_debug logging before gl.send
        gl.send(caller, share)
    ...
    def resolve_and_distribute(self) -> None:
        if self.status != OPEN:
            raise gl.vm.UserError("market not open")
        # TODO: add gl.emit_debug logging at start of this method
    ...
    def resolve(self) -> None:
        ...
        self.status = RESOLVED
`,
  task: "Add `gl.emit_debug(...)` calls to three locations: (1) in `place_bet()` after the bet is stored, logging the sender and bet details; (2) at the start of `resolve_and_distribute()`, logging the current status; (3) in `claim_winnings()` before `gl.send`, logging the recipient and amount.",
  hints: [
    "`gl.emit_debug(msg)` takes any string — use f-strings to include runtime values like addresses and amounts.",
    "In `place_bet`: `gl.emit_debug(f\"place_bet: {gl.message.sender_address} bet {outcome} with {gl.message.value} wei\")`",
    "In `claim_winnings`: `gl.emit_debug(f\"claim_winnings: sending {share} wei to {caller}\")` then call `gl.send`.",
  ],
};
