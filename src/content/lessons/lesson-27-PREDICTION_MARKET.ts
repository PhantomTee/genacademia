...
const content: LessonContent = {
  ...
  explanation: `## Lesson 27 — Upgradable Contracts

Smart contracts are immutable by default — once deployed, their logic is fixed. The **proxy pattern** works around this: you store an \`impl_address\` that points to the current implementation contract, and a \`version\` counter that tracks how many times the contract has been upgraded.

The core upgrade function is owner-gated:
...
In a full proxy setup, calls to unknown methods would be forwarded to \`impl_address\` using \`gl.call_contract\`. For PredictX we use the simpler pattern of storing the address for reference and emitting an event so off-chain indexers can track the upgrade history.

Keeping the version counter also gives users an easy way to verify they're interacting with the contract version they expect — a \`get_version()\` call is much cheaper than inspecting bytecode.`,
...
class PredictionMarket(gl.Contract):
    ...
    @gl.public.write
    def upgrade(self, new_impl: Address) -> None:
        pass  # TODO: owner-only; set self.impl_address = new_impl; increment self.version
    ...
    def resolve(self) -> None:
        ...
        self.status = RESOLVED
`,
  task: "Implement `upgrade(self, new_impl: Address) -> None` so only the owner can call it: check ownership, then set `self.impl_address = new_impl` and increment `self.version`. Implement `get_version()` to return `self.version`.",
  hints: [
    "Always check `gl.message.sender_address == self.owner` before any privileged state mutation.",
    "Increment `self.version` with `self.version += 1` after setting the new implementation address.",
    "Full implementation: `if gl.message.sender_address != self.owner: raise gl.vm.UserError(\"unauthorized\"); self.impl_address = new_impl; self.version += 1`",
  ],
};
