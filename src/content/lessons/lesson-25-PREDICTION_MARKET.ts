...
const content: LessonContent = {
  ...
  explanation: `## Lesson 25 — CAPSTONE: PredictX Payments

You now have the complete payment stack: payable bets that accumulate \`total_pot\`, \`gl.send\` for payouts, and an oracle for on-chain price verification. This capstone lesson ties them together with a single \`resolve_and_distribute()\` function that resolves the market and immediately pays all winners in one transaction.

The function needs to:
1. Call the AI leader to get the verdict
2. Set market status to RESOLVED
3. Iterate over all winners and calculate each one's share
4. Send each winner their GEN
...
def resolve_and_distribute(self) -> None:
    ...
    if winning_total == 0:
        return  # no winners — pot stays in contract

    for addr in self.winners:
        record = self.bets.get(addr, None)
        if record and record.outcome == self.resolution:
            share = record.amount * self.total_pot // winning_total
            self.claimed[addr] = True
            gl.send(addr, share)
\`\`\`

The \`winners: DynArray[Address]\` list is populated in \`place_bet\` when each bet is placed, giving you an iterable list of all bettor addresses for the payout loop.`,
...
class PredictionMarket(gl.Contract):
    ...
    @gl.public.write
    def resolve_and_distribute(self) -> None:
        pass  # TODO: resolve using run_nondet_unsafe, set status RESOLVED,
              # then iterate self.winners and gl.send each winner their proportional share
    ...
    def resolve(self) -> None:
        ...
        self.status = RESOLVED
`,
  task: "Implement `resolve_and_distribute()`: call `_leader_resolve()` via `run_nondet_unsafe` for the verdict, set `self.status = RESOLVED`, then iterate `self.winners` and call `gl.send` with each winner's proportional share of `total_pot`.",
  hints: [
    "Start with `run_nondet_unsafe` to get the verdict, store it in `self.resolution`, and set `self.status = RESOLVED`.",
    "Calculate `winning_total` by summing `.amount` for all bets whose `.outcome == self.resolution`, then iterate `for addr in self.winners`.",
    "For each winner: `share = record.amount * self.total_pot // winning_total; self.claimed[addr] = True; gl.send(addr, share)` — skip addresses that have already claimed or bet the wrong side.",
  ],
};
