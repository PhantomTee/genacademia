export const HINTS: Record<number, [string, string, string]> = {
  ...
  6: [
    "Call the LLM with: `result = gl.nondet.exec_prompt(prompt)`. The result is a string.",
    "Write a clear, specific prompt. Ask for a simple YES/NO or a specific value, not a paragraph explanation.",
    "The exec_prompt call happens on the leader validator first, then validators re-run it and check equivalence. The prompt must be deterministic enough for their results to match.",
  ],
  7: [
    "Pass `response_format='json'` to `exec_prompt` to get a structured dict back: `result = gl.nondet.exec_prompt(prompt, response_format='json')`.",
  ...
  18: [
    ...
    "The image_data can be passed directly to exec_prompt: `result = gl.nondet.exec_prompt(prompt, images=[image_data])`.",
    "Screenshots can be large. Crop or specify a viewport if only a specific section is relevant to reduce AI processing cost.",
  ],
  19: [
    "Pass images to the LLM: `gl.nondet.exec_prompt(prompt, images=[image_bytes])`. Multiple images supported.",
  ...
  22: [
    "Send GEN from the contract using `gl.send(recipient_address, amount)`. The contract must have sufficient balance.",
    "Always check the contract has enough balance before sending: `if self.total_balance < amount: raise gl.vm.UserError('insufficient funds')`.",
    "Update your internal balance tracking after a transfer to keep state consistent with the actual on-chain balance.",
  ],
  23: [
    "Initialize vector storage: `self.search: gl.VectorStorage = gl.VectorStorage()`. Add items with `.add(text, metadata)`.",
    "Search semantically with `.search(query, top_k=5)`. Returns a list of (score, metadata) tuples.",
    "Store the item ID or key in metadata so you can look it up by index after a semantic search.",
  ],
  24: [
    "Call another contract with `gl.call_contract(address, method_name, args)`. The address must be a deployed contract.",
    "The return type of `gl.call_contract` is determined by the called method's signature. Annotate your variable accordingly.",
    "Contract-to-contract calls count toward gas. Keep them minimal and cache results in state where possible.",
  ],
  25: [
    "Combine payable methods (lesson 21), value transfers (lesson 22), and contract calls (lesson 24) in a single flow.",
    "Use events (if available) or emit_debug to trace the full payment flow during testing.",
    "Edge cases: what if the recipient rejects the transfer? What if the called contract reverts? Handle these with try/except.",
  ],
  26: [
    "Generate a random number with `n = gl.get_random_u8()`. Returns a uint8 (0-255) deterministically agreed by validators.",
    "For a larger range, generate multiple bytes and combine: `r = gl.get_random_u8() | (gl.get_random_u8() << 8)` for a u16.",
    "Random numbers are consensus-safe because all validators will compute the same value given the same block context.",
  ],
  27: [
  ...
  28: [
    "Use `gl.emit_debug(message)` anywhere in your contract to write to the Studio debug logs. Visible in the Studio UI.",
    "Log the values of key variables at each step: `gl.emit_debug(f'Status: {self.status}, Balance: {self.balance}')`.",
    "emit_debug calls are stripped from production and don't affect gas. Use them liberally during development.",
  ],
  29: [
    "Implement `__receive_message__(self, message: str) -> None` to handle messages sent from other contracts.",
    "Implement `__receive_value__(self, amount: u256) -> None` to handle GEN sent directly to the contract address.",
    "Both special methods should validate the sender and amount before taking action. Don't trust arbitrary messages.",
  ],
  30: [
